...
# Purpose:  Prevents an Ubuntu host from suspending,
#           hibernating, or hybrid-sleeping. Useful for kiosk
#           machines, lab workstations, build agents, signage,
#           or any always-on endpoint where sleep states cause
#           outages.
#
...
#     When this script is run as root by an MDM/CM tool with
#     no user session, those steps are skipped and a warning
#     is logged. Re-run the user-session portion as the
#     desktop user, or deploy the gsettings/xset steps via a
#     per-user login hook.
...
#     system-wide. All activity is logged to $log AND streamed
#     to the console. Exit codes:
