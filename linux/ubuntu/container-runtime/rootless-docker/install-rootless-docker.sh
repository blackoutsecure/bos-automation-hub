...
#           repository and configures it to run in rootless
#           mode for a target non-root user on Ubuntu. Rootless
#           Docker runs the daemon and containers in a user
#           namespace, which removes the need to add the user
#           to the privileged "docker" group and reduces the
