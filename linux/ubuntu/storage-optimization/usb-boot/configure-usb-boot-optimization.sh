...
#           disk (HDD, SSD, or large flash drive) so that
#           random writes are minimised, sequential reads are
#           batched, and the device does not spin down or get
#           hammered by unnecessary background writes.
#
...
#   in non-interactive deployments:
...
#     activity is logged to $log AND streamed to the console.
#     Exit codes:
...
#   zramswapfile         - /etc/default/zramswap defaults file
#   ioschedrule          - Udev rules file for I/O scheduler selection
#   zram_target_percent  - PERCENT setting for zram-tools (% of RAM)
# =============================================================
