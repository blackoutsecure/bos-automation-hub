...
#
#           Some workloads, vendor install scripts, and legacy
#           shell code rely on bash-isms while still invoking
#           `/bin/sh`. On Ubuntu, /bin/sh defaults to dash for
#           boot-time speed and POSIX strictness, which breaks
#           those scripts. This script flips /bin/sh -> bash
#           and verifies the change.
#
# What the script does:
#   1. Verifies whether /bin/sh already resolves to bash and
#      skips reconfiguration when it does.
#   2. Sets the debconf selection `dash dash/sh boolean false`
#      and runs `dpkg-reconfigure -f noninteractive dash`,
#      which re-points /bin/sh -> bash.
