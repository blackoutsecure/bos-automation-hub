...
# Purpose:  Configures Ubuntu to boot with the legacy cgroups
#           v1 layout by adding the kernel parameter
#           `systemd.unified_cgroup_hierarchy=0` to GRUB and
#           regenerating the GRUB configuration.
#
#           Some workloads (older container runtimes, certain
#           monitoring agents, legacy Java tooling, and some
#           Kubernetes node images) require cgroups v1. Modern
#           Ubuntu releases default to cgroups v2 (unified
#           hierarchy). This script flips the kernel cmdline
#           back to v1 and updates GRUB so the change persists
#           across reboots.
#
# What the script does:
#   1. Backs up /etc/default/grub to a timestamped path.
#   2. If `systemd.unified_cgroup_hierarchy=0` is missing from
#      GRUB_CMDLINE_LINUX_DEFAULT it is appended; otherwise
#      the file is left untouched.
#   3. Runs `update-grub` to regenerate /boot/grub/grub.cfg
#      so the bootloader matches the on-disk config.
#
# Modes:
