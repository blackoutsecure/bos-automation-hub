...
#
# Security notes:
#   The legacy NodeSource one-liner `curl ... | sudo -E bash -`
#   pipes a remote shell script straight into root. This script
#   performs the same setup steps explicitly:
#     - pinned signing key in /etc/apt/keyrings
#     - signed-by= APT source entry (no apt-key)
#     - non-interactive apt-get install
#   The result is auditable, idempotent, and safe to re-run
#   from management tooling.
#
# Deployment:
...
#     All activity is logged to $log AND streamed to the
#     console. Exit codes:
